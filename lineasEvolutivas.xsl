<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <xsl:template match="/lineas_evolutivas">
        <html>
            <head>
                <title>Líneas Evolutivas Pokémon</title>
                <link rel="stylesheet" type="text/css" href="../web/css/lineasEvolutivas.css"/>
            </head>
            <body>
                <h1 class="titulo">Líneas Evolutivas Pokémon</h1>
                <xsl:for-each select="linea_evolutiva">
                    <xsl:variable name="bgColor">
                        <xsl:choose>
                            <xsl:when test="pokemon/tipo='Planta'">#e6f7e6</xsl:when>
                            <xsl:when test="pokemon/tipo='Fuego'">#ffe6e6</xsl:when>
                            <xsl:when test="pokemon/tipo='Tierra'">#e0b740</xsl:when>
                        </xsl:choose>
                    </xsl:variable>
